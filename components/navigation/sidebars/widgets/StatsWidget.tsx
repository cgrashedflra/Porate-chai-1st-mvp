  ...
  return (
    <WidgetCard title={title} icon={icon} viewAll={viewAll}>
      <ul className="grid grid-cols-3 gap-2">
        {stats.map((s) => (
          <li key={s.label} className="rounded-xl border border-slate-100 p-3">
      ...
      {progress && (
        <div className="mt-4">
          ...
          <div className="h-2 overflow-hidden rounded-full bg-indigo-100" role="progressbar" aria-valuenow={progress.percent} aria-valuemin={0} aria-valuemax={100} aria-label={progress.label}>
