...
export function PromoCard({ title, body, cta, imageSrc, className }: Props) {
  return (
    <div className={cn("glass-card relative overflow-hidden rounded-2xl bg-linear-to-br from-primary-50 via-primary-100/70 to-skyblue-100/60 p-5 shadow-md shadow-primary-500/10", className)}>
      <h3 className="max-w-[80%] text-lg font-semibold leading-snug text-primary-700">{title}</h3>
      {body && <p className="mt-2 max-w-[75%] text-sm text-neutral-500">{body}</p>}
      ...
      {imageSrc ? (
        ...
        <div aria-hidden="true" className="pointer-events-none absolute -bottom-4 -right-4 grid h-24 w-24 place-items-center rounded-full bg-white/50 text-primary-300">
